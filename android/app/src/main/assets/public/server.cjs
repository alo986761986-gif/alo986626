...
var activeDevices = {
  ...
  "smart-tv-bedroom": {
    ...
    isPlaying: false,
    progress: 120,
    lastUpdated: Date.now() - 2e5
  }
};
var collaborativeTracks = [
  { id: "track-2", title: "Midnight City", artist: "M83", votes: 4, addedBy: "Alessandro", timestamp: Date.now() - 36e5 },
  { id: "track-4", title: "Starlight", artist: "Muse", votes: 2, addedBy: "Sofia", timestamp: Date.now() - 18e5 }
];
var collaborativeMessages = [
  { id: "msg-1", user: "Alessandro", text: "Ragazzi ho aggiunto Midnight City, che ne pensate?", timestamp: Date.now() - 36e5 },
  { id: "msg-2", user: "Sofia", text: "Adoro! Voto subito la traccia \u{1F496}", timestamp: Date.now() - 175e4 }
];
app.post("/api/recommendations", async (req, res) => {
  ...
  if (offlineMode) {
    return res.json({
      ...
      aiExplanation: "Modalit\xE0 Offline attiva. Ti mostriamo solo i brani scaricati localmente nella tua libreria di Beatfy."
    });
  }
  ...
  - Cronologia recente: ${history ? history.map((h) => h.title).join(", ") : "Nessuna"}

  Genera 4 raccomandazioni di canzoni o podcast dal catalogo (puoi inventare titoli affascinanti o usare classici) con una breve spiegazione del perch\xE9 si adatta al loro mood.
    ...
    "recommendations": [
      {
        ...
        "justification": "Spiegazione in italiano del perch\xE9 \xE8 consigliata"
      }
    ],
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              aiExplanation: { type: import_genai.Type.STRING }
            },
    ...
    recommendations: [
      {
        ...
        artist: "Neon Dreamer",
        genre: "Electronic",
        justification: `Visto il tuo interesse per ${preferredGenres && preferredGenres[0] ? preferredGenres[0] : "la musica elettronica"}, questo brano synthwave elever\xE0 la tua concentrazione.`
      },
      {
        id: "rec-2",
...
app.post("/api/voice-command", async (req, res) => {
  ...
  Determina l'azione voluta e restituisci un oggetto JSON con queste propriet\xE0:
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              targetType: { type: import_genai.Type.STRING },
              targetName: { type: import_genai.Type.STRING },
              feedbackMessage: { type: import_genai.Type.STRING }
            },
  ...
  let responsePayload = {
    ...
    targetType: null,
    targetName: null,
    feedbackMessage: "Non sono sicuro di aver capito il comando vocale, ma prover\xF2 ad aiutarti!"
  };
...
app.post("/api/sync/update", (req, res) => {
  ...
  const fifteenMinutesAgo = Date.now() - 15 * 60 * 1e3;
